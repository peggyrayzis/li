...
Debug flags:
FlagDescriptionLI_DEBUG_HTTPLog request status + URLsLI_DEBUG_MESSAGESLog messaging queryId + variablesLI_DEBUG_MESSAGES_RESPONSELog response previews (may include PII)LI_DEBUG_QUERY_IDSLog queryId discovery progressLI_DEBUG_DUMPDump HTML/bundles/settings payloads to /tmp (advanced)
...
Network flags:
FlagDescription-nNumber of results (default: 20)--allPaginate through all results (until exhausted)--ofList a connection's connections (username, URL, or URN)--fastFaster pacing with adaptive slowdown on rate limits--confirmRequired for bulk operations like --accept-all
